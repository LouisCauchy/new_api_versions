...
def tickets():
    ...
    logger.info('Adding {} movies for user'.format(input_movies))
    ...
    if new_user:
        ...
        db.session.add(user)
        db.session.commit()
        logger.info('New user {} was added to db.'.format(user.id))
        movies_to_add = input_movies
        logger.info(movies_to_add) 
    else:
        ...
        if not movies_to_add and bool(user_movies):
            logger.info('All tickets already in db\n')
            return 'All tickets already in db\n'
        
    for movie in movies_to_add:
        ticket = Ticket(user.id, movie, datetime.now(), datetime.now())
        logger.info('{} is added to db for user {}'.format(movie, user.id))
