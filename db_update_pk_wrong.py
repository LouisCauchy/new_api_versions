...
#databases = {'vkino' : Vkino, 'planetakino': PlanetaKino} 
...
def write(true_tmdb_idx, db_name, column_name, movie_status, logger = None):
    ...
    for tmdbid, pkid in true_tmdb_idx:
        if db_name == 'vkino_movies':
            ...
            new_movie = PlanetaKino(pkid, tmdbid, movie_status)
            try:   
                db.session.add(new_movie)
                db.session.commit()
            except Exception as e:
...
class Update():
    ...
    def check_updates(self):
        ...
        if self.db_name == 'vkino_movies':
            actual_movies = Vkino.query.filter_by(is_actual = 1).all()
            current_premieres_ids = set([movie.vkino_id for movie in actual_movies])
        else:
            actual_movies = PlanetaKino.query.filter_by(is_actual = 1).all()
            current_premieres_ids = set([movie.planetakino_id for movie in actual_movies])
        ...
        if len(current_premieres_ids - new_premieres_movies_ids) > 0:
            ...
            for item in removed:
                if self.db_name == 'vkino_movies':
                    ...
                    removed_item = PlanetaKino.query.filter_by(planetakino_id = item).first()
        ...
        if len(new_premieres_movies_ids - current_premieres_ids) > 0:
            ...
            for movie in added:
                if self.db_name == 'vkino_movies':
                    ...
                    added_item = PlanetaKino.query.filter_by(planetakino_id = movie).first()
        ...
        for movie in none_movies:
            ...
            if self.db_name == 'vkino_movies':
                ...
                not_matched_movie = PlanetaKino(movie, None, 0)
            try:
