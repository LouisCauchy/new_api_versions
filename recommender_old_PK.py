...
class Recommender():
    ...
    def form_characteristics(self):
        if self.table_name == 'vkino_movies': 
            premieres = [(movie.tmdb_fk_id, movie.vkino_id) for movie in Vkino.query.filter(Vkino.tmdb_fk_id != None , Vkino.is_actual == 1).all()]
        else:
            premieres = [(movie.tmdb_fk_id, movie.planetakino_id) for movie in PlanetaKino.query.filter(PlanetaKino.tmdb_fk_id != None , PlanetaKino.is_actual == 1).all()]
